...
from models import Integration, IntegrationType, Workflow
...
async def google_oauth_callback(
    ...
    # Auto-create the integration. The OAuth callback has no auth header, so
    # we attribute the integration to the first registered user (parbaked's
    # local admin) — that's how callme's pre-migration code worked too.
    admin = session.exec(select(User).order_by(User.created_at).limit(1)).first()
    integration = Integration(

...
- ``GET  /api/live/ws``                    — WebSocket: real-time call events.
- ``POST /api/live/{call_id}/transfer``    — Transfer a call to the admin's
   registered phone number via Twilio.

The REST snapshot of active calls is served by ``routes/api/calls.py``
under ``/api/calls/live``.
"""
...
async def live_events_ws(ws: WebSocket) -> None:
    ...
    # parbaked owns auth — validate the JWT against parbaked's signing secret.
    try:
        from parbaked.auth.jwt import decode_jwt
        from parbaked.runtime import _get_active

        decode_jwt(token, _get_active().config)
    except Exception:
        await ws.close(code=4001, reason="Unauthorized")
        return
